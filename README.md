...
**RoleUp** は、AIを活用したロールプレイ型トレーニングツールです。

チャット形式で顧客役AIと模擬対応を行い、対応終了後にAIが会話内容を分析して具体的なフィードバックを返します。

- **難易度を3段階**（初級・中級・上級）から選択可能
- **実務頻出シナリオ**（解約引き止め・請求トラブル・クレーム対応など）を収録
- **RAG（検索拡張生成）** を採用し、社内マニュアルなどのPDFナレッジを参照しながら応答を生成
- 難易度・シナリオを**ランダム選択**することも可能
...
コールセンター業務では、ロールプレイ練習の機会が構造的に不足しがちです。
トレーナーの確保・時間調整が必要なため、新人研修期間に十分な反復練習ができないケースが多くあります。
また、経験者にとっても難易度の高いシナリオを繰り返し練習できる場が少ないという現場課題があります。

本ツールはその課題に対し、**AIが相手役を担うことでトレーナー不在でも・いつでも・何度でも練習できる環境**を提供することを目的に開発しました。
